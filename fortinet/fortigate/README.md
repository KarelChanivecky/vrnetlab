...
| `FOS_SKIP_PRODUCT_CHECK` | `false` | `true`, `false` | Skips validation that the image reports the expected product family and satisfies `FOS_PRODUCT_VERSION`. General FortiOS version detection still runs during bootstrap. |
| `FOS_PRODUCT_VERSION` | unset | `8`, `8.0`, `7.2.6`, `7.2.6.465`, or an inclusive range such as `7.2-8.0.1` | Constrains the reported product version. Missing dotted components are wildcards; the optional fourth component is the build number. |
